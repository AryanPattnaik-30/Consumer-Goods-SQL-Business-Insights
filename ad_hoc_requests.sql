...
SELECT 
    ...
    COUNT(DISTINCT CASE 
        WHEN fsm.fiscal_year = 2020 THEN fsm.product_code 
    END) AS product_count_2020,

    COUNT(DISTINCT CASE 
        WHEN fsm.fiscal_year = 2021 THEN fsm.product_code 
...
-- Request 5: Generate a report which contains the top 5 customers who received an 
...
SELECT 
    fpid.customer_code,
    dc.customer,
    ROUND(AVG(fpid.pre_invoice_discount_pct), 4) 
...
SELECT 
    ...
    ROUND(
        SUM(fsm.sold_quantity * gp.gross_price), 
...
SELECT 
    ...
    ROUND(
        SUM(fsm.sold_quantity * gp.gross_price) / 1000000,
        2
    ) AS gross_sales_mln,
    ...
    ROUND(
        SUM(fsm.sold_quantity * gp.gross_price)
        /
        SUM(SUM(fsm.sold_quantity * gp.gross_price)) 
            OVER () 
        * 100,
        2
    ) AS percentage
